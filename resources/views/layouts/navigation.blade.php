<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if (auth()->user()->hasRole(1))
                        ...
                        <x-nav-link :href="route('FoodPackages.food-packages')" :active="request()->routeIs('FoodPackages.*') || request()->routeIs('food-packages.*')">
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <!-- Product Stock Management (Managers and Employees) -->
            @if (auth()->user()->hasRole(1) || auth()->user()->hasRole(2))
                <x-responsive-nav-link :href="route('product-stock.index')" :active="request()->routeIs('product-stock.*')">
                    {{ __('Voorraadbeheer') }}
                </x-responsive-nav-link>
            @endif
            ...
            @if (auth()->user()->hasRole(1))
                <!-- Manager can see food packages -->
                <x-responsive-nav-link :href="route('FoodPackages.food-packages')" :active="request()->routeIs('FoodPackages.*') || request()->routeIs('food-packages.*')">
                    {{ __('Voedselpakketten overzicht') }}
                </x-responsive-nav-link>
            @elseif(auth()->user()->hasRole(3))
                <!-- Volunteer can see food packages -->
                <x-responsive-nav-link :href="route('volunteer.food-packages')" :active="request()->routeIs('volunteer.food-packages*')">
                    {{ __('Voedselpakketten overzicht') }}
                </x-responsive-nav-link>
            @endif
        </div>
